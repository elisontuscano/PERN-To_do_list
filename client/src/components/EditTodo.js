...
const EditTodo = ({todo}) =>{
    ...
    const updateDescription = async e => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/todos/${todo.tid}`, {
                method : "PUT",
    ...
    return (
        <Fragment>
            <button type="button" class="btn btn-warning" data-toggle="modal" data-target={`#id${todo.tid}`}  >
            Edit
            </button>

            <div class="modal fade" id={`id${todo.tid}`} onClick ={()=> setDescription(todo.description)}  >
